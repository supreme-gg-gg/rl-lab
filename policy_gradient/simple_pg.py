...
def train(env_name='CartPole-v1', hidden_sizes=[32], lr=1e-2,
    ...
    # the obs and act (tensors) you should plug in are the dataset collected according to currenct policy
    def compute_loss(obs, act, weights):

        '''
        This is not a loss function in the supervised learning definition.
        In policy gradients, we only care about average return.
        The loss function (decreasing) means nothing.
        '''

        # 1. Get policy distribution for obs and compute log prob of act taken under the policy
        # This quantifies how likely the policy was to choose this action in that state (log pi(a,s))
        logp = get_policy(obs).log_prob(act) 
        
        # when working with Normal distrib do log_prob(act).sum(axis=-1)        

        # 2. Maximise the weighted log prob of high-reward actions (weights = rtg) -- log pi() * R
        # Emphasize actions with higher rewards, and -ve because SGD/Adam minimise func by default
        # Then you take the average of all actions taken in the batch (sum(D)/|D|)
        return -(logp * weights).mean()
    
    optimizer = Adam(logits_net.parameters(), lr=lr)

    # Training policy for an entire epoch (full cycle of experience collection over multiple episodes)
    def train_one_epoch():
        ...
        optimizer.zero_grad()
        # All act and obs and ret are used to compute loss, "on-policy" training
        # We have transformed PG ascent into a loss function that can be minimised
    ...
    def reward_to_go(rews):
        ...
        # start from latest reward is easier to accumulate from end
        for i in reversed(range(n)):
            # rtg at time step i is rew at i + rtg at next time step (like recursion)
